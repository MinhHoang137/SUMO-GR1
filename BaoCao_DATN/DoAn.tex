...
\SetAlgorithmName{Giải thuật}{giải thuật}{DANH MỤC GIẢI THUẬT}
...
\usepackage{tikz} % Vẽ biểu đồ use case / hoạt động (bản nháp)
\usetikzlibrary{shapes.geometric, arrows.meta, positioning, fit, calc, shapes.multipart}
% Hình người (actor) kiểu UML cho biểu đồ use case
\tikzset{
  actor/.pic={
    \draw[thick] (0,0) circle (4pt);
    \draw[thick] (0,-4pt) -- (0,-16pt);
    \draw[thick] (-7pt,-8pt) -- (7pt,-8pt);
    \draw[thick] (0,-16pt) -- (-6pt,-26pt);
    \draw[thick] (0,-16pt) -- (6pt,-26pt);
  },
  uc/.style={draw, ellipse, align=center, inner sep=2pt, minimum width=2.8cm, minimum height=0.9cm, font=\small},
  ucbox/.style={draw, rounded corners, align=center, font=\small, inner sep=6pt},
  flow/.style={draw, rectangle, rounded corners, align=center, font=\small, inner sep=5pt, minimum width=3cm},
...
\def \AUTHOR{ĐẶNG MINH HOÀNG}
\def \EMAIL{Hoang.DM225719@sis.hust.edu.vn}
\def \INSTRUCTOR{ThS. Nguyễn Tiến Thành}
\def \DATE{HÀ NỘI, 06/2026}
\def \TOPIC{Hiển thị giao thông trong không gian 3D từ dữ liệu mô phỏng SUMO}
