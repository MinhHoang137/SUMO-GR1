...
\begin{algorithm}[H]
	...
	\label{alg:edge}
	\KwIn{Đường gấp khúc $P_0,\dots,P_{n-1}$; bề rộng làn $w$}
	\KwOut{Danh sách đỉnh lưới hai mép làn}
	\For{mỗi đỉnh $i$ của đường gấp khúc}{
		Tính hướng đi $d$ tại $P_i$\;
		\Indp đầu/cuối: theo đoạn kề; đỉnh giữa: phân giác hai đoạn\;
		\Indm $r \leftarrow \mathrm{cross}(d, -\vec{y})$ \tcp*{vector vuông góc trong mặt phẳng ngang}
		$dist \leftarrow w/2$\;
		\If{$i$ là đỉnh giữa}{
			$dist \leftarrow (w/2)/\cos\alpha$ \tcp*{$\alpha$: nửa góc cua, chống bóp mesh}
		}
		Phát hai đỉnh $P_i + r\cdot dist$ và $P_i - r\cdot dist$\;
	}
	Nối các cặp đỉnh liên tiếp: mỗi đoạn sinh hai tam giác\;
\end{algorithm}
...
\begin{algorithm}[H]
	...
	\label{alg:junction}
	\KwIn{Đa giác nút giao $V$; vector độ cao $\vec{h}$}
	\KwOut{Lưới khối ba chiều của nút giao}
	Chuẩn hóa $V$ về thứ tự chiều kim đồng hồ\;
	$T \leftarrow \mathrm{EarClip}(V)$ \tcp*{tam giác hóa, giữ được hình lõm}
	\If{$T$ chưa đầy đủ}{
		$V \leftarrow \mathrm{ConvexHull}(V)$ \tcp*{đa giác tự cắt $\rightarrow$ bao lồi}
		$T \leftarrow \mathrm{EarClip}(V)$\;
	}
	Mặt trên $\leftarrow T$\;
	Mặt dưới $\leftarrow T$ dời theo $\vec{h}$ (đảo chiều pháp tuyến)\;
	\ForEach{cạnh của $V$}{
		Thêm hai tam giác tạo mặt bên\;
	}
\end{algorithm}
...
chuyển sang xử lý xác xe thay vì trả về dòng giao thông.
