...
def build_simulation_csv_path(output_dir: str = "result", has_ped: bool = False) -> str:
	"""Create a CSV path named `Simulation-<date>-<hour>.csv`.

	Example: `result/Simulation-2026-02-04-15-30-05.csv`
	If the file already exists, appends `-1`, `-2`, ...
	"""
	ts = datetime.now().strftime("%Y-%m-%d-%H-%M-%S")
	if has_ped: base = f"Simulation-{ts}-ped"
	else: base = f"Simulation-{ts}"
	path = os.path.join(output_dir, f"{base}.csv")
	if not os.path.exists(path):
		return path
	idx = 1
	while True:
		cand = os.path.join(output_dir, f"{base}-{idx}.csv")
	...
	def on_step(self, step_index: int, departed_ids: Iterable[str], arrived_ids: Iterable[str]) -> list[VehicleTripRow]:
		...
		for vehicle_id in arrived_ids or []:
			...
			if depart_step is None:
				# If a vehicle existed before we started logging, we can't know its depart step.
				continue
			row = VehicleTripRow(vehicle_id=str(vehicle_id), depart_step=int(depart_step), arrival_step=int(step_index))
			self._writer.writerow([row.vehicle_id, row.depart_step, row.arrival_step, row.travel_steps])
			self._travel_steps_sum += int(row.travel_steps)
			self._travel_steps_count += 1
			written.append(row)

		# Ensure data is persisted even if the process crashes.
		self._file.flush()
		return written
